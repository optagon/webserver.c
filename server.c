...
bool load(FILE* file, BYTE** content, size_t* length) 
{
    ...
    while(true)
    {
        ...
        int read = fread(buffer, sizeof(BYTE), 8, file);
        
        
        if(read > 0)
        {
            
            //stores the adress of the fist of those bytes in *content
            *content = malloc(ftell(file));
            
            //stores the number of bytes in *lenght
            *length= ftell(file);
...
const char* lookup(const char* path)
{
    ...
    {
        //look up for dot character where doc_type is followed
 ...
 */
bool parse(const char* line, char* abs_path, char* query) //STILL ONDOING
{
        ...
        char* version = strtok(NULL, token);
    
        //copy tokens into method, requesttarget and httpversion
        char method[strlen(sp) + 1];
        strcpy(method, sp);
        char requesttarget[strlen(target) + 1];
        strcpy(requesttarget, target);
        char httpversion[strlen(version) + 1];
        strcpy(httpversion, version);
